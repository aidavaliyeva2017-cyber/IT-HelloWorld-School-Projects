...
class Sniper (Animate):
    ...
    def shot (self):
        if time.time () - self.reloaded_time > 3:
            ...
            dx = self.window.bill.center_x - x
            dy = self.window.bill.center_y - y
            ...
            speed = 7
            new_bullet = Sniper_bullet (self.window, dx * speed, dy * speed, x,y)
    ...
    def update (self):
        ...
        if self.window.bill.center_y  < self.center_y:
            if self.window.bill.center_x < self.center_x:
                ...
                self.shot()
                rot_angle = math.atan2(distance_y, distance_x)
                rot_angle_degree =math.degrees(rot_angle)
                self.angle = rot_angle_degree -35
            else:
                self.texture = self.sniper_left_angle
                self.shot()
                rot_angle = math.atan2(-distance_x, distance_y)
                rot_angle_degree =math.degrees(rot_angle)
                self.angle = rot_angle_degree -35
        else:
            if self.window.bill.center_x < self.center_x:
...
class Bullet (arcade.Sprite):
    def __init__(self, window):
        ...
        self.change_x = 0
        self.window = window 
        self.shoot_sound = arcade.load_sound("animate/sounds/shoot.wav")
...
class My_game (arcade.Window):
    def __init__ (self, SCREEN_WIDTH, SCREEN_HEIGHT, SCREEN_TITLE):
        ...
        #self.shoot_sound = arcade.load_sound("animate/sounds/shoot.wav")
        #self.sniper_sound = arcade.load_sound("animate/sounds/sniper.wav")
...
#hw: when shooter looks left right it should not look up. not higher than 180. not look up higher. 